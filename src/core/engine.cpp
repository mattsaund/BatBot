...
RouteDecision Engine::resolve(const Request& request) {
    ...
    const bool has_fallback = config_.has_expert(Subject::Fallback);
    ...
    if (decision.source == RouteSource::Model && has_fallback
        && config_.routing.min_confidence > 0.0F
        && decision.confidence < config_.routing.min_confidence) {
    ...
    const std::string wanted(subject_name(decision.subject));
    if (has_fallback && config_.routing.use_fallback_expert) {
        decision.subject = Subject::Fallback;
        decision.source  = RouteSource::Fallback;
    ...
    const std::vector<Subject> available = config_.configured_experts();
    if (available.empty()) {
