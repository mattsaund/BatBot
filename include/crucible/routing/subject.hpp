...
// The nine fundamental subjects Crucible delegates to, plus the metadata the
...
/// Everything Crucible knows about a subject that is not user configuration.
struct SubjectInfo {
    Subject          subject;
...
/// 42% to 74% on Crucible's own benchmark: a small instruct model follows a
