...
//
// The table of compute backends Crucible can run on.
//
// A "runtime" is one loadable ggml backend: a shared library that teaches
// llama.cpp how to talk to a piece of hardware. Crucible does not compile any of
...
/// A backend Crucible knows how to build and load.
///
...
///
/// Every backend Crucible knows about, including the ones this machine cannot
...
/// Could this backend exist on the machine Crucible is running on?
///
...
/// Returns nothing for a backend Crucible does not manage, which is not an
