...
//
// BatBot is started inside a directory and is about that directory, so history
...
/// The directory BatBot was started in, and where its history is kept.
struct Project {
    ...
    /// The project containing `cwd`, which is simply `cwd` -- BatBot does not
...
public:
    ...
    /// completes, so merely opening BatBot does not litter the history.
