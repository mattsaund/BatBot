...
//
// The first thing BatBot does that leaves the machine, which is why it is off
...
// works in six months. `curl` rather than libcurl: BatBot deliberately builds
...
///
/// The one BatBot asks for is a line of its own beginning `SEARCH:`. A text
/// protocol rather than the chat template's own tool-calling because BatBot
